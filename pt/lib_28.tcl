################################################################################
set LIBSET 20160428
set s_path [list . /proj/deneb1/wa/$USER/scr/sta /proj/deneb1/libset/DENEB_TOP.1.0A.$LIBSET/db ]
...
foreach SingleLib [list slow0p81v125 typ0p9v50 typ0p9v110 fast0p99vn40 fast0p99v125 slow0p81vn10] {
  ...
  foreach iRAMIP [ls /proj/deneb1/libset/DENEB_TOP.1.0A.$LIBSET/db/*_${SingleLib}*.db] {
    if { [string match "*u_lut*" $iRAMIP]  || [string match "*u_udm*" $iRAMIP] || [string match "*u_dpar*" $iRAMIP]  || [string match "*u_dpaw*" $iRAMIP]  || [string match "*nd_isl*" $iRAMIP]  ||  \
...
# STD Cells are already included in link_lib_ram_$SingleLib So this parameter has no use
  set link_lib_std_$SingleLib ""
